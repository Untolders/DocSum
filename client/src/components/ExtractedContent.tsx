...
export const ExtractedContent: React.FC<ExtractedContentProps> = ({ extractedText, fileName }) => {
  const [isExpanded, setIsExpanded] = useState(false);
  const [copied, setCopied] = useState(false);

  const wordCount = extractedText.content.split(/\s+/).filter(w => w.length > 0).length;
  const charCount = extractedText.content.length;
  const previewText = extractedText.content.substring(0, 300) + (extractedText.content.length > 300 ? '...' : '');
    ...
    <div className="bg-white rounded-xl shadow-lg border p-6">
      <div className="flex items-center justify-between mb-4">
        <div className="flex items-center space-x-2">
          ...
          <h3 className="text-xl font-bold text-gray-900">Extracted Content</h3>
        </div>
        ...
        <div className="flex items-center space-x-4">
          <div className="flex items-center space-x-4 text-sm text-gray-600">
            ...
            {extractedText.pageCount && (
              <div>{extractedText.pageCount} pages</div>
            )}
            {extractedText.confidence && (
              <div className="flex items-center space-x-1">
                <span>OCR Confidence:</span>
                <span className={`font-medium ${
                  extractedText.confidence > 90 ? 'text-green-600' : 
                  extractedText.confidence > 70 ? 'text-yellow-600' : 'text-red-600'
                }`}>
      ...
      {copied && (
        <div className="mb-4 text-green-600 text-sm text-center bg-green-50 py-2 rounded-lg">
